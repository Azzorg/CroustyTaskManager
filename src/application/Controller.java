...
public class Controller {
						...
						try {
							if(Main.listeTacheAFaire.get(j).getAffecte().getIdPersonne() == pActual.getIdPersonne())
							{
								...
								((Button)task.getChildren().get(4)).addEventHandler(ActionEvent.ACTION, event_2 ->  //action sur le bouton edit
								{
									...
									try {
										System.out.println("La tache en cours d'édition est la tache" +Main.tActual.getIdTache());
										Stage stage = null;
										Parent root = null;
	...
	private void handleButtonAction(ActionEvent event) throws IOException {
		...
		if (event.getSource() == task_valid) {
			if(task_name.getText()==null || task_content.getText()==null || task_state.getSelectionModel().isEmpty() || task_priority.getSelectionModel().isEmpty())
			{
